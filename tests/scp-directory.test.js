/**
 * @file scp-directory.test.js
 * @description SCP アダプタのディレクトリ転送（putDirectory / getDirectory）統合テスト
 *
 * 事前準備:
 *   docker compose up -d sftp
 *
 * テスト実行:
 *   npm test -- --testPathPattern=scp-directory
 */
 ...
 * @param {ReturnType<typeof createScpAdapter>} adapter
...
describe('SCP ディレクトリ転送', () => {
	/** @type {ReturnType<typeof createScpAdapter>} */
	let adapter;
	...
	describe('Round-trip: putDirectory → getDirectory', () => {
		const uploadLocalDir = path.join(TMP_BASE, 'macscp-scp-dir');
		const downloadLocalDir = path.join(TMP_BASE, 'macscp-scp-dir-downloaded');
		const remoteDir = '/upload/scp-dir-test';
	...
	describe('進捗コールバック', () => {
		const progressLocalDir = path.join(TMP_BASE, 'scp-progress-dir');
		const remoteProgressDir = '/upload/scp-progress-test';
		const downloadProgressDir = path.join(TMP_BASE, 'scp-progress-dir-downloaded');
